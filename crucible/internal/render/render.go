...
func Render(template string, varLookup expression.VariableLookup, funcCall expression.FunctionCall) (any, error) {
	...
	if len(matches) == 1 && isEncompassed {
		...
		result, err := expression.Evaluate(template[start+3:end-3], varLookup, funcCall)
		if err != nil {
			return "", err
		}
		return result, nil
