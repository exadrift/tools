...
import (
	...
	"github.com/exadrift/tools/crucible/internal/sequence"
...
func NewExecutor(cfg *config.Config, hostIdent string, sequencePath string) (*Executor, error) {
	...
	if isLoopback {
		...
		hostname, err := cfg.Hostname(hostIdent)
		if err != nil {
			return nil, err
		}
		if hostname == "" {
		...
		port, err := cfg.Port(hostIdent)
		if err != nil {
			return nil, err
		}
		if port == 0 {
			return nil, fmt.Errorf("the host port cannot be 0")
		}

		username, err := cfg.Username(hostIdent)
		if err != nil {
			return nil, err
		}
		if username == "" {
		...
		keyPath, err := cfg.KeyPath(hostIdent)
		if err != nil {
			return nil, err
		}

		knownHostFile, err := cfg.KnownHostsFile(hostIdent)
		if err != nil {
			return nil, err
		}
