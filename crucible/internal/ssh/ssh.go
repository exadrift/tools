...
func (s *SshSession) Connect() error {
	// if the underlying client has already been established don't do it again
	if s.client != nil {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("unable to establish ssh connection for user@host:port \"%s@%s\"\n%w", s.username, host, err)
	}
