...
import (
	...
	"github.com/exadrift/tools/crucible/internal/cmdsession"
	"github.com/exadrift/tools/crucible/internal/config"
	"github.com/exadrift/tools/crucible/internal/functions"
	"github.com/exadrift/tools/crucible/internal/log"
	"github.com/exadrift/tools/crucible/internal/render"
	"github.com/exadrift/tools/crucible/internal/ssh"
	"github.com/exadrift/tools/crucible/internal/utils"
...
type ExecutionInstance struct {
	...
	ExecContext          *kv.Store // context accumulated through execution ()
	HostContext          *kv.Store // per host config context
	lock                 sync.Mutex
...
func (ei *ExecutionInstance) variableLookup(key string) (any, error) {
	...
	return store.Get(kv.ParseNamespaceString(key)...), nil
}
...
func (ei *ExecutionInstance) whenSatisfied(action *Action) (bool, error) {
	...
	whenResult, err := expression.Evaluate(action.When, ei.variableLookup, functions.Call)
	if err != nil {
		return false, fmt.Errorf("unable to evaluate when clause: %s\n%w", action.When, err)
	}

	return expression.IsTruthy(whenResult), nil
}
...
func (ei *ExecutionInstance) Execute(action *Action) error {
	...
	if action.Iterate != "" {
		iterableResult, err := expression.Evaluate(action.Iterate, ei.variableLookup, functions.Call)
		if err != nil {
	...
	for {
		...
		if exitCode == 0 {
			...
			if action.PostProcess != "" {
				postProcess, err := expression.Evaluate(action.PostProcess, ei.variableLookup, functions.Call)
				if err != nil {
		...
		untilResult, err := expression.Evaluate(action.Until.Condition, ei.variableLookup, functions.Call)
		if err != nil {
			return fmt.Errorf("unable to evaluate until condition: %w", err)
		}
		if expression.IsTruthy(untilResult) {
			break
		}
	...
	if action.FailWhen != "" {
		failWhenResult, err := expression.Evaluate(action.FailWhen, ei.variableLookup, functions.Call)
		if err != nil {
			return fmt.Errorf("unable to evaluate failWhen condition %s: %w", action.FailWhen, err)
		}

		if expression.IsTruthy(failWhenResult) {
...
func (ei *ExecutionInstance) sync(action *Action) error {
	...
	username, err := ei.config.Username(ei.hostIdent)
	if err != nil {
		return err
	}

	hostname, err := ei.config.Hostname(ei.hostIdent)
	if err != nil {
		return err
	}

	port, err := ei.config.Port(ei.hostIdent)
	if err != nil {
		return err
	}

	keyPath, err := ei.config.KeyPath(ei.hostIdent)
	if err != nil {
		return err
	}

	return ssh.Rsync(username, hostname, port, keyPath, syncAction.Src, syncAction.Dest)
}
