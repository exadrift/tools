...
expression is an expression evaluation framework which can deal with the following characteristics
...
expression has strict and predictable rules when it comes to type inference.
...
```
import (
  "github.com/exadrift/go/expression"
)

result, err := expression.Evaluate("(10 + 1) * 3 / 100", nil, nil)
if err != nil {
  log.Fatal(err)
}
...
because expression is a generic evaluation framework, it does not define any variable storage mechanisms, nor does it define any builtin functions.  both variable data store, and function implementation are left to the implementer, allowing for maximum flexibility.

here is an example using variable lookup, using [kv](https://github.com/exadrift/go/kv) as the variable storage/lookup backend:
```
import (
  "github.com/exadrift/go/expression"
...
vLookup := func(key string) (any, error) {
  ...
  // then use that key to access the value on the kv object
  return store.Get(ParseNamespaceString(key)), nil
}

result, err := expression.Evaluate(".first_key.second_key * 33 + 1", vLookup, nil)
// result should be of type float64 and equal to (101 * 33) + 1
```

the above example uses `kv`, however the implementor is welcome to use any means of variable value retrieval, which will in turn be substituted for the variable name during evaluation.

function lookup is performed in a very similar fashion.  when function lookup occurs, variables, etc. have already been evaluated by `expression`, thus the implementor need not perform any additional lookup, just locate, validate, and execute the function.  below is a rudimentary example of a function lookup to illustrate its usage in the simplest form - building on the previous example:
...
fLookup := func(name string, args ...any) (any, error) {
  ...
  result, err := expression.Evaluate("len(.first_key.third_key) * 5", vLookup, fLookup)
...
```
// array indexes
x := expression.Evaluate("myfunc('abc')[-1]")

// mapping access
x := expression.Evaluate("myfunc('abc').def.ghi")

// mixed access
x := expression.Evaluate("myfunc('abc').def[0].ghi[3]")
```

## helpers
`expression` comes with a few utility functions to aid in processing of evaluated results
